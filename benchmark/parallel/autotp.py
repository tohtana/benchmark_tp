...
class AutoTPStrategy(BaseTPStrategy):
    """
    DeepSpeed AutoTP tensor parallelism strategy with optional data parallelism.
    ...
    def setup(
        ...
        When dp_size > 1, sets up 2D parallelism by:
        1. Creating explicit TP and DP process groups
        2. Configuring DeepSpeed with the TP group
        3. Letting DeepSpeed handle DP via its data parallel mechanism
        """
        import deepspeed
        ...
        # With DP+TP, only dp_size ranks process different data batches
        # (ranks in the same TP group process the same batch)
